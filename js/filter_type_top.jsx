...
/***/ function(module, exports, __webpack_require__) {
	...
	var MainCenter = React.createClass({
		...
		componentDidMount: function componentDidMount() {
			...
			Promise.all([getdata, active]).then(function (values) {
				console.log(values);
				...
				mui(".header").on("tap", ".text_queren", function () {
					...
					for (var i = 0; i < _this.state.data.length; i++) {
						...
						if (_this.state.data[i].active == 1) {
							...
							mui.fire(plus.webview.getWebviewById(self.callEventWebviewId), "newType", _this.state.data[i]);
							mui.back();
							break;
						}
					}
		...
		parseData: function parseData(data, active) {
			var areaList = Array();
			if (active == "全部") {
				areaList.push({
					name: "全部",
		...
		render: function render() {
			...
			if (_this.state.ajaxState == "complete") {
				if (_this.state.data.length != 0) {
					content = React.createElement(ChooseList, { ChooseListData: _this.state.data, callback: this.state.changeFunc });
				} else {
	...
	var ChooseList = React.createClass({
		...
		componentDidMount: function componentDidMount() {
			...
			mui(thisNode).on("tap", "li", function () {
				...
				var name = this.childNodes[0].childNodes[0].innerText;
				var sub = this.childNodes[0].childNodes[1].innerText;
		...
		render: function render() {
			...
			for (var i = 0; i < _this.props.ChooseListData.length; i++) {
				contentList.push(React.createElement(
					"li",
					null,
					React.createElement(
						"span",
						{ className: _this.props.ChooseListData[i].active ? 'diqu_icon-choosed lefticon' : 'diqu_icon lefticon' },
						React.createElement(
							"span",
							null,
							_this.props.ChooseListData[i].name
						),
						React.createElement(
							"i",
							null,
							_this.props.ChooseListData[i].sub ? "(" + _this.props.ChooseListData[i].sub + ")" : ""
						)
					)
				));
			}
